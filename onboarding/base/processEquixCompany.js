    ...
    if (target !== undefined || mapping.defaultValue !== undefined) {
      if (key.includes("[index]")) {
        applicantDetails.forEach((applicant, applicantIdx) => {
          ...
          // Thay thế "[index]"
          if (key.includes("officeHolders[index]")) {
            replacedKey = replacedKey.replaceAll(
              "officeHolders[index]",
              `officeHolders[${applicantIdx}]`
            );
          }
          if (key.includes("beneficialOwners[index]")) {
            replacedKey = replacedKey.replaceAll(
              "trustees[index]",
      ...
      if (key.includes("isSupplied")) {
        const type = Array.isArray(value)
          ? "array"
          : typeof value;
        if (!key.includes("officeHolders") && !key.includes("beneficialOwners")) {
          const tfn3 = getValueArray(data, "company_tfn");
          ...
          applicantDetails.forEach((applicant, applicantIdx) => {
            ...
            } else if (key.includes("beneficialOwners[index]")) {
              replacedKey = key.replaceAll(
                "beneficialOwners[index]",
                `beneficialOwners[${applicantIdx}]`
              );
              result[replacedKey.replace("[index]", `[0]`)] = normalizeValue(
                value,
                type
      ...
      } else if (key.includes("assetContributionIsImmaterial")) {
        const is_10000 = getValueArray(data, "is_10000");
        value = is_10000 ? false : true;
        const type = Array.isArray(value)
          ? "array"
          : typeof value;
        result[key] = normalizeValue(value, type);
